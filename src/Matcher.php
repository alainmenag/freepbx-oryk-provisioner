...
class Matcher extends Service
{
	 ...
	 *                             to what was asked for, so one resource covers
	 *                             every client on the profile -- and a vendor that
	 *                             does not put the MAC at the front, or anywhere,
	 *                             can still be named exactly.
	 *   phone.cfg                 compared against the request with the MAC taken
	 *                             off the front, so the ordinary case can be typed
	 *                             as the tail on its own.
	 *
	...
	public function matchResource($profileId, $requested, $suffix, array $values)
	{
		...
		foreach ($stmt->fetchAll(PDO::FETCH_ASSOC) as $resource) {
			...
			// Filenames are matched without regard to case throughout.
			if (strcasecmp($this->template->renderTemplate($name, $values), $requested) === 0) {
				return $resource;
			}

			// Held rather than returned: a rendered name is what its author wrote out
			// in full, and it wins over one that only matches the tail.
	 ...
	 * 0004f282e824-phone.cfg, 00:04:f2:82:e8:24-phone.cfg and 0004f282e824.cfg are
	 * all one client asking. What is left is the part a resource is named after --
	 * phone.cfg, and .cfg for the main config.
	 *
	 * A filename that does not begin with this client's MAC comes back unchanged:
	 * it is either meant literally or meant for somebody else.
	 *
	 * Read a character at a time, stopping the moment twelve hex digits are in
	 * hand, because a pattern that allows a dot between them will also take the
	 * dot of the extension: [mac].cfg has to come back as `.cfg`, not `cfg`.
	 *
	...
	public function resourceSuffix($filename, $mac)
	{
		...
		for ($i = 0, $length = strlen($filename); $i < $length; $i++) {
			...
			// A separator, but only once there is something for it to separate: a name
			// starting with one is not a MAC.
		...
		// The dot of an extension belongs to the name that is left -- [mac].cfg is
		// `.cfg` -- where a dash or underscore is only the vendor's way of joining
		// the two, and part of neither.
	 ...
	 *
	 * matchResource() read backwards. A name is matched either as it renders or as
	 * the tail of a request with the MAC taken off the front, so the request that
	 * reaches this resource is one of:
	 *
	 *   {{device.mac}}-phone.cfg  renders to 0004f282e824-phone.cfg, asked for as
	 *                             it stands.
	 *   phone.cfg                 has no MAC to render, so the phone asks for
	 *                             0004f282e824-phone.cfg and resourceSuffix()
	 *                             takes the MAC back off. Joined by nothing when
	 *                             the name is an extension of its own (.cfg), by a
	 *                             dash otherwise.
	 *
	 ...
	 * twelve hex digits -- 000000000000-directory.xml -- would render the wrong
	 * client, so those rows get the filename and no link.
	 *
	...
	public function resourceRequest($name, array $values, $mac)
	{
		...
		// Somebody else's twelve hex digits, asked for exactly as they stand. It
		// is served, and it is not linkable: the endpoint would read that MAC as
		// the client.
		if ($carries !== '') {
			return ['filename' => $rendered, 'url' => ''];
		}

		// No placeholders and no MAC of its own, so the phone asks for the MAC and
		// this name joined and the endpoint takes the MAC back off again.
		...
		// Nothing in the name says which client is asking, so the request says it
		// the endpoint's other way.
	 ...
	 * only against the name as typed: with no client there is no MAC to take out
	 * and nothing to render a templated name against, so a file meant to be
	 * fetched this way is named exactly what the vendor asks for.
	 *
	 * Narrowed to type = 'file' and not to file_size IS NOT NULL -- the same set
	 * today and a different question: what may be handed to a caller who has said
	 * nothing about who they are is what its author *declared* a file.
	 *
	 * Case is the collation's business rather than LOWER()'s: the column is
	 * case-insensitive, so a plain comparison ignores case and can use the index.
	 *
	 * Two profiles carrying the same firmware is the ordinary case rather than an
	 * ambiguity worth refusing, so the lowest id wins.
	 *
	 * @param string $requested Filename as it was asked for.
	 *
	 * @return array<string, mixed>|null The resource, or null when none answers.
	 ...
	 *
	 * The type is the second argument because the kinds want different answers to
	 * an extension nothing here recognises: a template is configuration text, a
	 * log is text a person is about to read, and an uploaded file is bytes
	 * somebody chose -- sending a firmware image as text is how it arrives
	...
	{
		// A colon is legal in a path segment and is how half the vendors separate a
		// MAC, so it is left as it is rather than escaped into something the
