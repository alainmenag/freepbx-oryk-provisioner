 ...
 * The clients table.
 *
 * A client is a MAC, the FreePBX device it stands for and the profile it is
 * served -- and the MAC is the only required part of that. clientByMac() is
 * the endpoint's way in, and the same row the editor reads.
 */
class Clients extends Service
{
	// Being switched on and off is the same here as on a profile -- see
	// src/Enabled.php.
	use Enabled;
	 ...
	 *
	 * The one thing the table must not show to say a client is authenticated is
	 * the token itself, so the question is answered in SQL and only the answer
	 * travels. A constant because the same expression is both selected and sorted
	 * on, and the two drifting apart would sort the column by something other than
	 * what it displays.
	 */
	 ...
	 *
	 * Subtracted by the database, not the browser: a DATETIME is written in the
	 * PBX's own clock and carries no zone, so a browser in another timezone
	 * working the age out itself would be hours off. Both sides of this
	 * subtraction are the database's clock.
	 *
	 * NULL when the client has never been answered, and stays NULL.
	 */
	 ...
	 * editor's Clients tab is filled -- the same rows read the same way rather
	 * than a second statement to keep in step. The resource editor's Clients tab
	...
	public function listClients()
	{
		...
		// not offered as a sortable heading sorts by MAC.
		$sortable = [
			'mac' => 'pc.mac',
			...
			'enabled' => 'pc.enabled',
			// Sorted by the timestamp rather than the age beside it: same fact one
			// subtraction apart, and the stored column is the one an index can reach.
			// A client never seen sorts first ascending, which is where the fleet is
			// read from.
		...
		// profile nothing points at comes back empty instead of as the whole list.
		...
		// what somebody arrives at this list already holding.
		if ($search !== '') {
	 ...
	 * profile is: the editor is a page of its own, so there is nothing left for it
	 * to wait on over AJAX.
	 *
	...
	{
		// The hash included: the editor shows it, so a save that leaves the field
		// alone writes back what was there, and emptying the field is the one
		// unambiguous way to take a token away. A hash rather than the secret is
		// what makes showing it tolerable.
	 ...
	 *
	 * One statement rather than three lookups: the whole of what rendering needs
	 * is one row wide. `pc.id` is on it because a stored log lives in a directory
	 * named after the client -- see LogRepo -- and the endpoint has only this row
	 * to learn that from.
	 *
	...
	public function saveClient($request)
	{
		...
		// A new client with no MAC needs a row before it can have an id.
		if (!$id && !$mac) {
			$stmt = $this->db->prepare(
		...
		// Refused rather than stored as typed, and said separately for each so the
		// message names the box to go back to. See address().
		...
		// Only when there is a MAC to be taken. Two clients without one are not a
		// collision: neither can be reached by a MAC.
		if ($mac !== null) {
		...
		// The Token field round-trips, so what comes back is either the hash it
		// was shown or something typed over it. **A colon tells the two apart**: a
		// password_hash() carries none, so a value with one has not been through
		...
		if ($token !== '' && strpos($token, ':') !== false) {
			...
			// password_hash() has no failure a caller can act on, so this is refused
			// rather than stored as something that would never verify.
			if ($tokenHash === null) {
	 ...
	 *
	 * **Validated rather than trimmed and written**, and that is the invariant the
	 * Clients list depends on: the private address is put into an `href` on the
	 * row, and a value filter_var() has agreed is an address cannot be a
	 * `javascript:` anything. Nothing else on a client is written into a URL the
	 * admin's own browser follows.
	 *
	 * IPv4 and IPv6 pass; a hostname does not, deliberately.
	 *
	 ...
	 * and is not unique, so the option carries both and is found by either.
	 *
	 ...
	 *
	 * A button on the row and not only a field on the page: switching a phone off
	 * is what somebody does to a client they are not otherwise editing.
	 *
	 * The write is the trait's, shared with a profile. What is this class's is the
	 * row -- a client that has gone is said in this table's own words, before
	 * anything is written rather than by an UPDATE that quietly matches nothing.
	 *
	 ...
	 * is being refused. **A PUT counts** -- a phone uploading its boot log was
	 * answered with a 200 like any other request.
	 *
	 * NOW() rather than a timestamp from PHP, because the column is compared
	 * against NOW() when the age is read.
	 *
	 * `updated_at = updated_at` is load-bearing: that column is
	 ...
	 *
	 * Nothing in here may fail a request: the phone has already been answered by
	 * the time this runs.
	 *
	 ...
	 * Remove a client.
	 *
	 * Nothing points at a client the way a client points at a profile, so there is
	 * nothing to refuse this for.
	 *
