 ...
 *
 * It can also be switched off, which is the same switch a client has one level
 * up: a disabled profile serves nothing to anybody, so every client assigned to
 * it is refused without any of them being touched.
 */
class Profiles extends Service
{
	...
	public function deleteProfile($id)
	{
		...
		// Resources go with it: unlike a client, a resource has no existence
		// apart from the profile that serves it. Their ids are read first because
		// an uploaded file is named after the resource it belongs to -- once the
		// rows are gone there is nothing left to say which files were theirs.
	 ...
	 *
	 * A disabled profile is still one of them: it is what a client being set up
	 * against a profile not yet in service should be assigned to. The state travels
	 * with the name so the select can say which ones are switched off.
	 *
